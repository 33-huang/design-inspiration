...
**添加 / 编辑条目后：**
1. **刷新页面**，条目是否还在？（排除 localStorage 缓存假象）
2. **图片是否显示？** Firebase 图片在国内无 VPN 时不显示属正常，不是 bug
3. **换无痕模式再刷新**，图片是否还在？（确认 data.json 已更新，不依赖缓存）

**图片不显示时的排查顺序：**
1. 先直接在浏览器打开 Firebase 图片 URL，确认能否加载 → 不能加载说明是网络/VPN 问题，不是代码问题
2. 能加载但页面不显示 → 检查 `resolveImg()` 返回值是否正确
3. 刷新后消失但刚保存时显示 → 检查 `getWriteBase()` 是否用了 raw CDN（见注意事项）
