...
**排查时走过的错误方向**：
- ❌ 怀疑 `saveData()` 写入失败 → 实际写入成功，问题在读取端
- ❌ 怀疑 localStorage 缓存没清 → 清了之后刷新还是复现，缓存不是根因
...
**排查时走过的错误方向**：
- ❌ 怀疑浏览器缓存了旧版 HTML → 加 `?v=2` 强制刷新后问题依旧
- ❌ 怀疑 GitHub token 未配置导致数据不加载 → 实际数据加载成功，条目可见，只有图片不显示
- ❌ 给 fetch 加 timeout 试图修复图片卡住 → 无效，根因是 Google 服务在国内不可达，不是超时问题
